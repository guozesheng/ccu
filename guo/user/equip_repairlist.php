...
<table width="100%" border="0" id="table_style_all" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
     <tr>
      <td><strong>&nbsp;仪器维修申请</strong>&nbsp;&nbsp;<a href="equip_repairlist.php?action=seek">申请列表</a> | <a href="equip_repairlist.php">申请历史</a></td>
     </tr>
     <tr>
      <td bgcolor="#FFFFFF">
	   <?php
	   ...
	   if ($_GET['action'] == "seek")
	   {
		   $csql->SetQuery("select * from #@__repair_ask where boss_id = '$_SESSION[boss]' and isallow != 1 ");
	   }
	   else 
	   {
		   $csql->SetQuery("select * from #@__repair_ask where boss_id = '$_SESSION[boss]' ");
	   }
	   $csql->Execute();
	   $rowcount=$csql->GetTotalRow();
	   if($rowcount==0)
	   echo "<tr><td>&nbsp;没有任何维修申请。</td></tr>";
	   else{	?>
	   ...
	   ?>
       	<tr class='row_content'>
        	...
        	<td>&nbsp;<a href="equip_repairshow.php?id=<?=$row['id']?>&ac=show">详细</a></td>
        </tr>
       <?php
	   $basicsql->close();
	   $bosssql->close();
