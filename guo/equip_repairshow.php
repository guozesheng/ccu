...
if ($ac=='save')
{
	...
	if ($baction == 1)
	{
		...
		$repairsql->Execute();
		$repairsql->close();
		Showmsg('已同意','equip_repair.php?action=do');
		exit ();
	}
	else if ($bbaction == 0)
	{
		...
		$repairsql->Execute();
		$repairsql->close();
		Showmsg('已拒绝','equip_repair.php?action=do');
		exit ();
	}
	exit ();
}
...
<table width="100%" border="0" id="table_style_all" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
     <tr>
      <td><strong>&nbsp;仪器维修详单</strong>&nbsp;&nbsp;- <a href="equip_repairlist.php?action=seek">申请列表</a> | <a href="equip_repairlist.php">申请历史</a></td>
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="0" id="table_border">
  ...
  <tr>
    ...
    <td>确认维修<input type="radio" name="baction" checked="checked" value="1" />&nbsp;拒绝<input type="radio" name="baction" value="0" /></td>
  </tr>
  <tr>
